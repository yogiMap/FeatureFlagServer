...
const groupSearch = async (req, res) => {
  ...
  try {
    ...
    const totalCountPromise = Group.countDocuments(query); // Находим кол-во результатов
    const searchPromise = groupSearchQuery({ query, page, limit }); // Находим результат
    ...
    res.status(200).json(message.success('GroupSearch ok', result));
  } catch (error) {
    const analyticsId = analytics('GROUP_SEARCH_ERROR', {
      error,
      body: req.body,
      entity: 'Group',
      user: userId,
    ...
    res.status(400).json(message.fail('GroupSearch error', analyticsId));
  }
};

export default groupSearch;

function groupSearchQuery({ query, page, limit }) {
  return Group.find(query)
    ...
    .exec()
    .then((docs) => {
      return message.success('Group found', docs);
    });
}
