...
const baseSearch = async (req, res) => {
  ...
  try {
    ...
    const totalCountPromise = Base.countDocuments(query); // Находим кол-во результатов
    const searchPromise = baseSearchQuery({ query, page, limit }); // Находим результат
    ...
    res.status(200).json(message.success('BaseSearch ok', result));
  } catch (error) {
    const analyticsId = analytics('BASE_SEARCH_ERROR', {
      error,
      body: req.body,
      entity: 'Base',
      user: userId,
      controller: 'baseSearch',
    });

    res.status(400).json(message.fail('BaseSearch error', analyticsId));
  }
};

export default baseSearch;

function baseSearchQuery({ query, page, limit }) {
  return Base.find(query)
    ...
    .exec()
    .then((docs) => {
      return message.success('Base found', docs);
    });
}
